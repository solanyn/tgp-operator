...
func TestClient_GetProviderName(t *testing.T) {
	...
	t.Run("should return provider name", func(t *testing.T) {
		...
		if name != "lambda-labs" {
			t.Errorf("Expected provider name to be 'lambda-labs', got: %s", name)
		}
	})
}
...
func TestClient_ListOffers(t *testing.T) {
	...
	t.Run("should return mock offers for testing", func(t *testing.T) {
		offers, err := client.ListOffers(ctx, "RTX3090", "us-west-2")
		if err != nil {
		...
		offer := offers[0]
		if offer.Provider != "lambda-labs" {
			t.Errorf("Expected provider to be 'lambda-labs', got: %s", offer.Provider)
		}
		if offer.GPUType != "RTX3090" {
			t.Errorf("Expected GPU type to be 'RTX3090', got: %s", offer.GPUType)
		}
		if offer.Region != "us-west-2" {
			t.Errorf("Expected region to be 'us-west-2', got: %s", offer.Region)
		}
...
func TestClient_GetPricing(t *testing.T) {
	...
	t.Run("should return pricing info", func(t *testing.T) {
		pricing, err := client.GetPricing(ctx, "RTX3090", "us-west-2")
		if err != nil {
			t.Errorf("Expected no error, got: %v", err)
		}
